...
# Iceberg Tags {#iceberg-tags}

启用 Iceberg 兼容性后，Paimon 的标签（Tag）也会同步到 [Iceberg Tags](https://iceberg.apache.org/docs/nightly/branching/#historical-tags)。
只有当所引用的快照在 Iceberg 表中存在时，标签才会被同步到 Iceberg。
