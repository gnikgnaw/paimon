...
**源码路径**: `paimon-core/src/main/java/org/apache/paimon/index/FileIndexProcessor.java` (L174-184)
...
```java
// BloomFilterFileIndex.java (L48-64)
...
public class BitmapGlobalIndex implements GlobalIndexer {
    ...
    public GlobalIndexReader createReader(...) {
        FileIndexReader reader = index.createReader(input, 0, indexMeta.fileSize());
...
V2 的 `Bitmap64DeletionVector` 为了与 Iceberg 的 Position Delete 格式兼容，采用了小端序存储。Iceberg 使用小端序，Paimon 为了兼容性采用了相同的字节序。读取时通过 `toLittleEndianInt(magicNumber)` 将读取的大端序 magic number 转换为小端序后再比较。
