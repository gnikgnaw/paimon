...
> **版本**：1.5-SNAPSHOT　**覆盖模块**：`paimon-api` / `paimon-core` / `paimon-flink` / `paimon-common`　**核对日期**：2026-06

**一句话定位**：这是一份"按源码核对过"的 Paimon 面试题库，覆盖 LSM Merge-Tree、Snapshot/Manifest、Merge 引擎、Flink 集成、Deletion Vector、查询优化、运维调优、Iceberg 对比八大主题。每道题不止给标准答案，更讲清"为什么这么设计、踩过哪些坑、收益与代价"——这正是面试官追问时真正想听的层次。它同时是 `source-analysis/` 全套专题文档的索引入口：每个主题板块末尾都标注了对应的深读文档。

**读完本题库你应能从容回答**：① 为什么 Paimon 选 LSM 而不是 Iceberg 的不可变文件，差异落在写放大还是读放大上；② 一次 commit 为什么可能产生两个 Snapshot，拆分换来了什么；③ 四种 MergeFunction / 四种 ChangelogProducer 各自在权衡什么、配错会怎样；④ Flink Sink 为什么 Committer 必须单并行度、Exactly-Once 靠哪三道闸；⑤ Deletion Vector 凭什么让主键表也能 rawConvertible；⑥ Universal Compaction 三级策略、反压、小文件治理在调什么参数、量化代价是多少；⑦ Paimon 与 Iceberg 在存储模型、流式更新、小文件理念上的根本分野。

> **使用约定**：
> - **难度标记**：🟢 基础 | 🟡 中级 | 🟠 高级 | 🔴 专家。建议按难度梯度准备，基础题答错会被直接 pass，高级/专家题用来拉开差距。
> - **答题结构**：每道题给「核心答案」（含为什么/坑/代价）→「源码证据」（精选片段 + `路径:行号`）→「面试口述建议」（可直接背诵的 60 秒版本）。
> - **源码行号**以本次（2026-06）核对为准，与旧稿不符处用「（已修正）」标注；类名/方法名保留英文原文。
> - **想深挖某个机制**，跳到对应主题板块末尾的「深入阅读」，里面指向 01~24 专题文档的具体章节。
...
### 九、设计决策总结
- [9.1 一张表看懂 Paimon 的关键设计取舍](#design-summary)

---

## 一、LSM Merge-Tree 核心原理

> **板块导读**：这组题考的是 Paimon 最底层的"地基"——为什么实时湖仓要用 LSM 而不是 B+Tree 或 Iceberg 那样的不可变文件。一句话抓住本质：**对象存储只能顺序追加、不能原地改写，LSM 把随机 upsert 转成"追加新文件 + 后台异步合并"，用可控、可异步收敛的读放大换取极低的写放大。** 面试时容易答得太浅（只会说"写优化"），区分度在于能不能讲清三个易错点：① Level 0 文件 key 范围重叠、是 LSM 的正常态，盲目追求清空 L0 反而阻塞写入；② 一个 Level 可以含多个 SortedRun（Level 0 每个文件算一个 run），也可以只有一个（高层）；③ sequenceNumber 决定数据新旧，被嵌进排序键里，理解错会以为"后写的不一定覆盖先写的"。
>
> **深入阅读**：LSM 数据结构、Level/SortedRun 不变式、写入与归并全链路详见 **01-核心存储引擎分析**（§3 LSM 数据结构、§4 写入路径、§5 读取路径）；Universal Compaction 三级策略与 IntervalPartition 的完整推导详见 **23-Compaction 全链路深度分析**。
...
> **板块导读**：这组题考的是 Paimon 的元数据与并发控制层。核心设计是**四层增量日志**：`Snapshot → ManifestList（base/delta/changelog 三分）→ ManifestFile → ManifestEntry`。三分法不是炫技，每一层都服务于一个真实的高频操作：全表扫描只读 base、流式消费只读 delta、CDC 订阅只读 changelog、快照过期只读 delta 就能判断"哪些文件是本快照引入的"。面试区分度在于能不能讲清两个反直觉的点：① 一次 commit 可能产生**两个** Snapshot（APPEND + COMPACT），因为两者冲突检测策略不同、拆开后 COMPACT 失败也不影响新数据落地；② `commitIdentifier`（Flink 下就是 checkpoint ID）是 Exactly-Once 去重的锚点，恢复后靠它判断"这批数据是否已提交"。
>
> **深入阅读**：Snapshot/Manifest 三层索引与 Scan 多级裁剪详见 **01-核心存储引擎分析 §12**；Catalog 与元数据生命周期详见 **03-Catalog 与元数据管理**；时间旅行、Tag/Branch 对快照的保护详见 **17-时间旅行与版本管理**。
...
```java
// 源码: paimon-core/src/main/java/org/apache/paimon/operation/FileStoreCommitImpl.java:290（已修正，旧稿标 288）
@Override
...
```java
// 源码: paimon-core/src/main/java/org/apache/paimon/operation/ManifestFileMerger.java:62（已修正，旧稿标 60）
...
```java
// 源码: paimon-core/src/main/java/org/apache/paimon/operation/ManifestFileMerger.java:138（已修正，旧稿标 136）
...
> **板块导读**：这组题考"相同主键的多条记录如何合并"——Paimon 主键表的灵魂。设计动机是把合并语义下沉到存储层（Compaction 时顺带合并），而不是丢给计算层，省去数据来回传输。四种 `MergeEngine`（deduplicate / partial-update / aggregation / first-row）覆盖了 CDC 去重、宽表汇聚、实时指标、日志去重四类场景。面试区分度集中在三个"魔鬼细节"：① PartialUpdate 的 **Sequence Group** 解决的是多源乱序——每组字段用各自的序列号字段独立判新旧，没配会数据错乱；② **LookupMergeFunction 的装饰器设计**——Compaction 时不读全部历史版本，只取"上一次合并结果"那一条高层记录，FirstRow 被豁免包装；③ Aggregation 的聚合器要区分 `agg`（正向）和 `retract`（撤回），20+ 内置函数走 SPI + Factory 组织。
>
> **深入阅读**：四种 MergeFunction、Sequence Group、Lookup 包装、聚合函数体系的完整源码剖析详见 **08-Merge 引擎与聚合函数**；局部列更新在 CDC 宽表场景的落地详见 **14-局部列更新与 CDC 数据集成**；Changelog 四种 producer 的产生时机与代价详见 **24-Changelog 机制全链路分析**。
...
```java
// 源码: paimon-api/src/main/java/org/apache/paimon/CoreOptions.java:3910（已修正，旧稿标 3807）
...
```java
// 源码: paimon-api/src/main/java/org/apache/paimon/CoreOptions.java:4051（已修正，旧稿标 3948）
...
> **板块导读**：这组题考 Paimon 怎么跟 Flink 的 Checkpoint 机制咬合，做到流式写入的 Exactly-Once。核心拓扑是 **Writer（多并行度）→ Committer（强制单并行度）**：Writer 只写数据不生成 Snapshot，Committer 在 `notifyCheckpointComplete` 时原子提交。Committer 必须单并行度，是因为 Snapshot 创建要全局唯一、`commitIdentifier` 要单调递增。面试区分度在于能否串清 Exactly-Once 的三道闸：① `snapshotState` 把待提交数据持久化进 Flink operator state（崩溃可恢复）；② `notifyCheckpointComplete` 才真正提交 Snapshot（用 NavigableMap 批量取 ≤ 当前 checkpoint 的数据）；③ 恢复后用 `commitIdentifier` 去重，跳过已提交的批次。另外两个高频追问：CDC 同步的 Schema 自动演进、Lookup Join 的缓存与增量刷新。
>
> **深入阅读**：Flink Source/Sink 拓扑、Exactly-Once 提交、CDC Sink 详见 **05-Flink 集成源码分析**；写入链路从 RowData 到落盘的完整流程详见 **15-Flink 写入 Paimon 源码流程分析**；CDC 与 Schema 演进的协同详见 **14-局部列更新与 CDC 数据集成** 与 **22-Schema 演进机制深度分析**。
...
**Schema 自动演进**的核心在于：
- 每条 CDC 记录（`CdcRecord` / `RichCdcRecord`）携带其字段信息
- 拓扑中插入一个 `UpdatedDataFieldsProcessFunction`（多表场景为 `MultiTableUpdatedDataFieldsProcessFunction`），它对比上游字段与当前表 schema，检测出新增列/类型拓宽
- 通过 `SchemaManager.commitChanges()` 发起 schema change，Paimon 的 schema evolution 基于递增的 schema ID，每次变更产生新的 schema ID，老数据文件按其写入时的 schema ID 反序列化
- 演进受兼容性约束：只允许"安全"的变更（加列、类型拓宽），不兼容变更会抛错而非静默丢数据

`RichCdcSinkBuilder` 相比 `CdcSinkBuilder` 携带了更完整的字段元信息，支持分库分表合并等高级特性。

**源码证据：**

```java
// CDC 同步入口构建器
// paimon-flink/paimon-flink-cdc/.../sink/cdc/CdcSinkBuilder.java
// paimon-flink/paimon-flink-cdc/.../sink/cdc/RichCdcSinkBuilder.java（携带字段元信息，支持分库分表合并）

// Schema 自动演进的检测算子（单表 / 多表两版）
// paimon-flink/paimon-flink-cdc/.../sink/cdc/UpdatedDataFieldsProcessFunction.java
// paimon-flink/paimon-flink-cdc/.../sink/cdc/MultiTableUpdatedDataFieldsProcessFunction.java
// 二者继承自 UpdatedDataFieldsProcessFunctionBase，最终调用 SchemaManager.commitChanges() 落地变更
```
...
> **板块导读**：这组题考"不重写文件就能行级删除 + 跳过不匹配行"的两件利器。**Deletion Vector（DV）**是一个 RoaringBitmap，标记数据文件里哪些行已删除——删除/更新不再重写整个文件，只写一个小 bitmap，写放大骤降；更妙的是它让主键表的 split 变成 `rawConvertible`，可以绕过 Paimon 的 Merge 直接喂给 Spark/Flink 的原生 Parquet/ORC reader。**文件索引（File Index）**则嵌在数据旁，按列做行级/文件级过滤。面试区分度：① DV 的 V1（32 位 RoaringBitmap，约 21 亿行上限）与 V2（64 位，从 Iceberg 移植，little-endian magic number）格式差异；② DV 让 PK 表 rawConvertible 的原理（旧记录标删而非物理删，文件+DV 自包含）；③ 四种索引（Bloom Filter / Bitmap / BSI / Range Bitmap）按"基数 × 查询类型"选型，选错会索引爆炸或失效。
>
> **深入阅读**：DV 的 V1/V2 实现、rawConvertible 转换、四种文件索引原理与选型详见 **04-DeletionVectors 与文件索引**（机制主讲）；索引在读路径上的应用、Bitmap 行级裁剪详见 **13-索引机制深度分析**；Predicate 下推与多级裁剪的查询侧视角详见 **12-查询优化与多维排序策略**。
...
```java
// 源码: paimon-core/.../deletionvectors/BitmapDeletionVector.java:36（MAGIC_NUMBER）、50（delete）
...
```java
// 源码: paimon-core/.../deletionvectors/Bitmap64DeletionVector.java:40（MAGIC_NUMBER）、64（delete）
...
```java
// 源码: paimon-core/.../deletionvectors/DeletionVector.java:97（read）、103/117（magic 分支）
...
> **板块导读**：这组题考"在海量文件里怎么少读数据"。Paimon 的查询优化是**四层渐进式过滤**：分区 → Manifest（partitionStats / bucket / level 范围）→ 数据文件（列 min/max）→ 行级（文件索引）。设计哲学是"成本递增、越早过滤越赚"——分区剪枝几乎零成本却能砍掉绝大多数数据，索引过滤成本高但放在最后。面试区分度：① 能不能把四层的过滤依据各自落到具体元数据字段上（而不是只说"会过滤"）；② **Z-Order/Hilbert** 解决的是多列联合查询的局部性——单列排序对其他列无效，空间填充曲线让多列 min/max 同时变紧凑；③ **LookupLevels** 为主键点查把列式文件转成本地 KV 索引，避免每次 Compaction 重读整文件。
>
> **深入阅读**：多维排序（Z-Order/Hilbert）、Manifest/文件级裁剪的完整实现详见 **12-查询优化与多维排序策略**；文件索引的内部结构与行级裁剪详见 **13-索引机制深度分析**；LookupLevels 点查、Lookup 文件远程化详见 **01-核心存储引擎分析 §6**。
...
```java
// 源码: paimon-core/.../mergetree/LookupLevels.java:131（公开入口）、146（按文件查找）
@Nullable
...
```java
// 源码: paimon-core/.../mergetree/LookupLevels.java:126
@Override
...
> **板块导读**：这组题考"生产环境怎么把表调稳"，最容易暴露候选人有没有真扛过线上。三个核心抓手：**Compaction 调参**（`num-sorted-run.compaction-trigger` 一个参数三重作用——触发频率、层数、归并内存，调大写更快但读更慢、峰值内存更高）、**Bucket 选择**（每桶一棵 LSM，过少则单桶巨大、并行度受限，过多则小文件泛滥，经验值单桶几百 MB ~ 几 GB）、**Snapshot 过期的多重保护**（数量 / 时间 / Tag / Consumer / Changelog 五重闸 + 先删数据文件再删 Snapshot 的原子顺序）。面试区分度在于能不能把"调这个参会怎样"量化成代价，而不是只背默认值。
>
> **深入阅读**：运维全套调优方案（监控指标、故障排查、独立 Compaction 作业）详见 **10-运维优化方案**；小文件成因与治理详见 **11-小文件治理机制**；Compaction 全链路与参数推导详见 **23-Compaction 全链路深度分析**；Bucket 模式与动态桶详见 **16-分桶机制原理与实践**；快照过期与 Tag/Consumer 保护详见 **17-时间旅行与版本管理**。
...
**Bucket 模式**（源码 `BucketMode` 枚举共五种，旧稿只列三种、已修正）：

| 模式 | 配置 | 适用 | 关键约束 |
|---|---|---|---|
| **HASH_FIXED** | `bucket = N`（N≥1） | 数据量可预估的主键/append 表 | 桶数固定，改桶需离线 rescale；读端可按 bucket key 做 bucket skipping |
| **HASH_DYNAMIC** | 主键表 + `bucket = -1` | 数据量波动、主键含全部分区字段 | 维护 hash→bucket 映射；不支持并发写、不支持读端 bucket skipping |
| **KEY_DYNAMIC** | 主键表 + `bucket = -1` 且跨分区更新 | 主键不含全部分区字段的跨分区 upsert | 用本地磁盘维护"主键→分区+桶"映射，启动须读全表主键建索引 |
| **BUCKET_UNAWARE** | append 表 + `bucket = -1` | append-only 表 | 无桶概念（数据写 bucket-0），读写并行度不受限 |
| **POSTPONE_MODE** | 主键表 + `bucket = -2` | 难以预估桶数、希望分区各异 | 后台自适应调整桶数，免去人工定桶 |

`bucket = -1` 在主键表上会根据是否跨分区更新自动落到 HASH_DYNAMIC 或 KEY_DYNAMIC；在 append 表上即 BUCKET_UNAWARE。`bucket = -2`（POSTPONE_BUCKET）是较新引入的自适应桶模式。

Bucket 数量直接决定 Flink 的写入并行度——每个 Writer 实例负责一个或多个 bucket，bucket 数量过少会让写入成为瓶颈。

**源码证据：**

```java
// 源码: paimon-common/src/main/java/org/apache/paimon/table/BucketMode.java:30-73
public enum BucketMode {
    HASH_FIXED,      // bucket = N，固定桶
    HASH_DYNAMIC,    // bucket = -1，主键表动态桶（hash→bucket 映射）
    KEY_DYNAMIC,     // bucket = -1，跨分区 upsert（key→partition+bucket 映射）
    BUCKET_UNAWARE,  // bucket = -1，append 表，全部写 bucket-0
    POSTPONE_MODE;   // bucket = -2，后台自适应
...
> "Bucket 是分区内的数据分片单位，每个 bucket 有独立的 LSM-Tree。数量太少导致单桶数据过大、Compaction 慢、写入并行度受限；太多则小文件泛滥。建议单桶几百 MB 到几 GB。源码里 BucketMode 有五种：HASH_FIXED 固定桶、HASH_DYNAMIC 主键动态桶、KEY_DYNAMIC 跨分区更新、BUCKET_UNAWARE 给 append 表、还有较新的 POSTPONE_MODE（bucket=-2）后台自适应。bucket=-1 在主键表会自动选 HASH_DYNAMIC 或 KEY_DYNAMIC，取决于是否跨分区更新。"
...
> **板块导读**：这组题考"为什么选 Paimon 而不是 Iceberg"，是选型类高频题。一句话抓本质：**两者元数据形式上几乎同构（Snapshot → ManifestList → Manifest → DataFile），分野在存储与更新模型——Paimon 用 LSM-Tree 把更新当第一公民、写入即治理；Iceberg 用不可变文件，更新靠 COW 重写或 MOR 删除标记、事后再 Rewrite 治理小文件。** 面试区分度：① 不要陷入"Iceberg 不支持更新"的误区（V2 有 Equality/Position Delete），要落到**更新性能**差异（COW 写放大 vs LSM 增量合并）；② 小文件治理理念的根本不同（自动异步 vs 显式 Action）；③ 流式更新能力（Paimon 原生 MergeFunction/Changelog vs Iceberg 依赖引擎层）。
>
> **深入阅读**：Paimon 与 Iceberg 的全面对比（存储、元数据、更新、生态、迁移）详见 **21-Paimon 与 Iceberg 全面对比**；Paimon 自身对 Iceberg 元数据的兼容层详见该文档的兼容性章节。
...
### 9.1 一张表看懂 Paimon 的关键设计取舍

把本题库各主题里反复出现的"为什么这么设计"压缩成一张决策表，面试时可以用它快速勾勒 Paimon 的设计哲学：

| 决策点 | 选择 | 取舍 / 代价 | 收益 |
|---|---|---|---|
| **存储数据结构** | LSM Merge-Tree（而非 B+Tree / 不可变文件） | 引入读放大，需后台 Compaction 持续收敛 | 写放大极低、纯顺序写，契合对象存储与高频 upsert |
| **Level 0 数据结构** | TreeSet（key 范围可重叠，按 seq 降序） | L0 文件多则点查/归并要逐个扫描 | flush 路径简单、最新数据天然在前；高层用 SortedRun 支持二分 |
| **Compaction 策略** | Universal（三级：size-amp / size-ratio / file-num） | 单参数 `compaction-trigger` 三重耦合（频率/层数/归并内存），调参需量化 | 写密集场景下写放大可控，参考 RocksDB 成熟设计 |
| **写入反压** | SortedRun 数超 `stop-trigger` 阻塞写入 | 写吞吐受限于 Compaction 速度 | 防止 L0 无限堆积导致读放大失控 |
| **元数据分层** | Snapshot → ManifestList(base/delta/changelog) → Manifest → Entry | 多层结构、Manifest 需定期合并 | 全表读 base、流读 delta、CDC 读 changelog、过期只读 delta，各场景精准最小读取 |
| **一次 commit 两个 Snapshot** | APPEND 与 COMPACT 拆分提交 | 单次 commit 可能多生成一个快照 | 冲突检测策略分离、COMPACT 失败不影响新数据落地、重试更简单 |
| **提交去重** | `commitIdentifier`（= Flink checkpoint ID）单调递增 | 依赖外部 ID 单调性 | 故障恢复后精确去重，保障 Exactly-Once |
| **Merge 语义** | 四种 MergeEngine + Sequence Group + 20+ 聚合 | 配置复杂度高，配错（漏 sequence-group）会数据错乱 | 合并下沉存储层，免计算层二次处理；覆盖去重/宽表/指标/日志 |
| **Lookup 合并** | LookupMergeFunction 装饰器，只取一条高层历史 | 额外 Lookup I/O 与本地 SST 磁盘 | 实时精确 changelog、复用任意 MergeFunction、避免读全部历史版本 |
| **行级删除** | Deletion Vector（标删而非重写） | 读端需叠加 DV 过滤；物理删除延迟到 Compaction | 写放大骤降；让主键表 split 变 rawConvertible，可走原生 reader |
| **文件索引** | Bloom / Bitmap / BSI / Range Bitmap 嵌入式 | 选型错（高基数用 Bitmap）会索引爆炸 | 行级/文件级精准跳过，按"基数 × 查询类型"匹配 |
| **查询过滤** | 分区 → Manifest → 文件 → 行 四层渐进 | 索引层成本高 | 成本递增、越早过滤越省，海量文件下避免全扫 |
| **Flink Sink 拓扑** | Writer 多并行度 + Committer 单并行度 | 单点 Committer 限制提交吞吐 | Snapshot 全局唯一、commitIdentifier 单调，简化分布式协调 |
| **小文件治理** | 写入即治理（Compaction 自动异步） | 占用后台 CPU/IO 资源 | 无需人工干预，区别于 Iceberg 的事后 Rewrite Action |

---

> **文档版本说明**：本题库基于 Apache Paimon 1.5-SNAPSHOT（master 分支）源码核对编写，核对日期 2026-06。各题源码行号以本次核对为准；与早期旧稿不符处已用「（已修正）」标注。题目按主题分类导航，每个板块「板块导读」末尾的「深入阅读」指向 `source-analysis/` 对应专题文档（01~24），可作系统化深读的索引。
