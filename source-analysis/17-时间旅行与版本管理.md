...
> 基于 Apache Paimon 1.5-SNAPSHOT 源码分析，commit: 7c93bd720
...
public class Snapshot implements Serializable {
    ...
    // ... 以及 record count, statistics, properties, nextRowId
}
```
...
- 清理使用"跳过集合"机制：收集左邻 Tag 和右邻最近 Snapshot 作为跳过边界
...
**为什么 LOOKUP/FULL_COMPACTION 模式的初始扫描要过滤 level**（第 158-166 行）:
- LOOKUP 模式: 初始扫描只读 level > 0 的数据，因为 level 0 数据会在后续 compaction 中产生 changelog
- FULL_COMPACTION 模式: 初始扫描只读最高 level 的数据，避免重复
...
|---------|----------|----------|----------|
| `DeltaFollowUpScanner` | 所有 Snapshot | DELTA | changelog.producer=none |
...
**ChangelogFollowUpScanner**（`snapshot/ChangelogFollowUpScanner.java`）:
...
**为什么跳过没有 changelog 的 Snapshot**: 在 LOOKUP/FULL_COMPACTION 模式下，只有 compaction 才产生 changelog。普通的 APPEND 提交不产生 changelog，跳过可以减少不必要的扫描。
...
| `snapshotRetainMin` | `1` | 最少保留多少个快照 |
| `snapshotTimeRetain` | `Long.MAX_VALUE` ms | 快照最大保留时间 |
| `snapshotMaxDeletes` | `Integer.MAX_VALUE` | 单次过期最多删除数量 |
